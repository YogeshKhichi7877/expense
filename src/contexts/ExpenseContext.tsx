...
// import { showSuccess, showError } from '../utils/customToast'; // Import custom toasts
...
//         loading: false, // ✅ FIXED: Turn off loading on success
//         error: null
//       };
//     case 'DELETE_EXPENSE':
//       return {
...
const expenseReducer = (state: ExpenseState, action: ExpenseAction): ExpenseState => {
  switch (action.type) {
    ...
    case 'ADD_EXPENSE':
      return { ...state, expenses: [...state.expenses, action.payload], loading: false, error: null };
    case 'DELETE_EXPENSE':
      return {
        ...state,
        expenses: state.expenses.filter(expense => (expense._id || expense.id) !== action.payload),
        loading: false,
        error: null
      };
    case 'UPDATE_EXPENSE':
  ...
  const actions = {
    loadExpenses: async () => {
      ...
      try {
        const expenses = await apiService.getExpenses();
        dispatch({ type: 'SET_EXPENSES', payload: expenses });
        // No success toast needed for loading initial data, it gets annoying
      } catch (error) {
    ...
    addExpense: async (expense: Omit<Expense, '_id' | 'id' | 'createdAt' | 'updatedAt'>) => {
      ...
      try {
        ...
        const msg = (error as Error).message;
        dispatch({ type: 'SET_ERROR', payload: msg });
        showError(msg || 'Failed to add expense');
        throw error; // Re-throw so the modal can handle closing state if needed
      }
    },
    ...
    updateExpense: async (id: string, expense: Partial<Expense>) => {
      ...
      try {
        ...
        const msg = (error as Error).message;
        dispatch({ type: 'SET_ERROR', payload: msg });
        showError(msg || 'Failed to update expense');
      }
    },

    deleteExpense: async (id: string) => {
      // Note: Delete confirmation is usually handled in the UI component BEFORE calling this
      dispatch({ type: 'SET_LOADING', payload: true });
      try {
        await apiService.deleteExpense(id);
        dispatch({ type: 'DELETE_EXPENSE', payload: id });
        ...
        const msg = (error as Error).message;
        dispatch({ type: 'SET_ERROR', payload: msg });
        showError(msg || 'Failed to delete expense');
      }
    }
  };
